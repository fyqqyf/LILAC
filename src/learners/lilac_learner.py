...
class LilacLearner(QLearner):
    def __init__(self, mac, scheme, logger, args):
        super(LilacLearner, self).__init__(mac, scheme, logger, args)
        self.args = args
        self.mac = mac
        self.logger = logger
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        for t in range(batch.max_seq_length):
            agent_outs, loss_, dis_loss_, mi_loss_ = self.mac.forward(
                batch, t=t, train_mode=True
        ...
        if t_env - self.log_stats_t >= self.args.learner_log_interval:
            ...
            self.logger.log_stat("loss_prior", reg_loss.item(), t_env)
            self.logger.log_stat("dis", dis_loss.item(), t_env)
            self.logger.log_stat("loss_mi", mi_loss.item(), t_env)
