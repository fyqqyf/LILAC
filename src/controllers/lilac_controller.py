...
class LilacMAC(BasicMAC):
    def __init__(self, scheme, groups, args):
        super(LilacMAC, self).__init__(scheme, groups, args)
    ...
    def select_actions(self,
        ...
        agent_outputs, _, _, _ = self.forward(ep_batch,
                                              t_ep,
                                              test_mode=test_mode)
    ...
    def forward(self, ep_batch, t, test_mode=False, train_mode=False):
        agent_inputs = self._build_inputs(ep_batch, t)  # (bs*n,(obs+act+id))
        avail_actions = ep_batch["avail_actions"][:, t]
        # (bs*n,(obs+act+id)), (bs,n,hidden_size), (bs,n,latent_dim)
        agent_outs, self.hidden_states, loss, dis_loss, mi_loss = self.agent.forward(
            agent_inputs,
            self.hidden_states,
            ep_batch["obs"][:, t],
        ...
        return agent_outs.view(ep_batch.batch_size, self.n_agents,
                               -1), loss, dis_loss, mi_loss
        ...
        if self.args.obs_last_action:  # True for QMix
            if t == 0:
                inputs.append(th.zeros_like(
                    batch["actions_onehot"][:, t]))  # last actions are empty
            else:
                inputs.append(batch["actions_onehot"][:, t - 1])
        inputs.append(batch["obs"][:, t])  # b1av
        if self.args.obs_agent_id:  # True for QMix
            inputs.append(
                th.eye(self.n_agents, device=batch.device).unsqueeze(0).expand(
                    bs, -1, -1))  # onehot agent ID

        # inputs[i]: (bs,n,n)
        inputs = th.cat([x.reshape(bs * self.n_agents, -1) for x in inputs],
                        dim=1)  # (bs*n, act+obs+id)
